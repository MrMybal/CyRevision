...
- Optional, project-scoped Jira Cloud, ClickUp, and CyTask integrations for searching work items, adding stable task links, auto-detecting them in pull requests, and applying confirmed or automatic completion transitions after merge; session tokens are never persisted.
...
```powershell
./scripts/build-release.cmd 0.1.26
```
