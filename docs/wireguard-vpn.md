...
Le VPN est un module indépendant. Un projet peut utiliser uniquement WireGuard, WireGuard + Sync, WireGuard + Git, ou les trois. CyRevision utilise les outils officiels de WireGuard et simplifie leur configuration ; il ne remplace pas le protocole.

## Assistant desktop

1. Ouvrez un projet puis l'onglet **VPN WireGuard**.
2. Cliquez sur **Détecter et configurer automatiquement**. CyRevision trouve l'installation officielle, crée une clé privée et une interface `cyrev-xxxxxxxx` propre au projet.
...
- Windows utilise uniquement `/installtunnelservice` et `/uninstalltunnelservice` pour le nom du projet ; Linux utilise `wg-quick up/down` sur le fichier du projet.
