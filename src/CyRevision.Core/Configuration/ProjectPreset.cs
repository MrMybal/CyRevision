...
public sealed record ProjectPreset(
    ...
    FeatureMaturity Maturity = FeatureMaturity.Stable)
{
    public bool IsPluginMode =>
        !string.IsNullOrWhiteSpace(PluginModeId) &&
        !string.IsNullOrWhiteSpace(ProviderPluginId);

    public string MaturityLabel => Maturity.ToString().ToUpperInvariant();
    public bool IsAlpha => Maturity == FeatureMaturity.Alpha;
    public bool IsBeta => Maturity == FeatureMaturity.Beta;
